...
function displayHelp(): void {
  ...
  console.log(
    "       --visitors: use custom a output visitor. (--visitors dictionary,createjs)"
  );
  console.log(
    "       --interactive: ask to write the output in case it was manually modified"
  );
...
export function run(): void {
  ...
  let visitors = args["visitors"]
    ? args["visitors"]
        .split(",")
        .map((name: string) => require(`./custom-visitors/${name}`).default)
    : [];
  ...
  if (overwrite && interactive) {
    console.error(
      "You can't have '--overwrite' and '--interactive' at the same time."
    );
    process.exit();
  }
  ...
  files.forEach((file) => {
    ...
    if (!overwrite && fs.existsSync(outputFile)) {
      ...
      if (previousLockTimestamp.getTime() !== stat.mtime.getTime()) {
        if (
          interactive &&
          !readlineSync.keyInYN(`"${fileTs}" has been modified. Overwrite it?`)
        ) {
          return;
        }
      }
